        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          >
            ...
            <div className="bg-white rounded-[2.5rem] p-8 border border-outline-variant shadow-sm text-center overflow-hidden relative group">
              ...
              <h2 className="text-2xl font-heading font-extrabold text-primary mb-1">{user.name}</h2>
              ...
              {menuItems.map((item) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <item.icon className={`w-5 h-5 ${activeTab === item.id ? "text-on-primary" : "text-primary opacity-60"}`} />
          ...
          >
            <AnimatePresence mode="wait">
              {activeTab === 'overview' && (
                ...
                >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                    {[
                      { label: "Active Orders", value: user.totalOrders, icon: Clock, color: "text-blue-600", bg: "bg-blue-50" },
                      { label: "Wishlist Items", value: user.wishlistCount, icon: Heart, color: "text-red-600", bg: "bg-red-50" },
                      { label: "Wellness Points", value: user.wellnessPoints, icon: Star, color: "text-yellow-600", bg: "bg-yellow-50" }
                    ].map((stat, idx) => (
                      <div key={idx} className="bg-white p-8 rounded-[2rem] border border-outline-variant shadow-sm flex flex-col items-center text-center group hover:border-primary/30 transition-colors">
                        <div className={`w-12 h-12 ${stat.bg} ${stat.color} rounded-2xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform`}>
                          <stat.icon className="w-6 h-6" />
                        </div>
                        <span className="text-3xl font-heading font-extrabold text-primary mb-1">{stat.value}</span>
                  ...
                  <div className="bg-white rounded-[2.5rem] border border-outline-variant shadow-sm p-8 md:p-12">
                    <div className="flex items-center justify-between mb-8">
                      <h3 className="text-2xl font-heading font-extrabold text-primary">Recent Orders</h3>
                      <button 
                      ...
                      <table className="w-full">
                        <thead>
                          <tr className="text-left border-b border-outline-variant">
                            <th className="pb-4 text-xs font-bold text-on-surface-variant uppercase tracking-widest">Order ID</th>
                            <th className="pb-4 text-xs font-bold text-on-surface-variant uppercase tracking-widest">Date</th>
                            <th className="pb-4 text-xs font-bold text-on-surface-variant uppercase tracking-widest">Status</th>
                            <th className="pb-4 text-xs font-bold text-on-surface-variant uppercase tracking-widest">Items</th>
                            <th className="pb-4 text-xs font-bold text-on-surface-variant uppercase tracking-widest text-right">Total</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-outline-variant/30">
                          {orders.map((order) => (
                            <tr key={order.id} className="group hover:bg-surface-container-low/30 transition-colors">
                              ...
                              <td className="py-6 text-sm text-on-surface-variant">{order.items} items</td>
                              <td className="py-6 font-bold text-primary text-right">{order.total}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                  ...
                  <div className="bg-primary/5 rounded-[2rem] p-8 flex flex-col md:flex-row items-center gap-6 border border-primary/10">
                    ...
                    <div className="flex-grow text-center md:text-left">
                      ...
                      <p className="text-sm text-on-surface-variant">We've implemented multi-factor authentication and high-end encryption to keep your medical data private and secure.</p>
                    </div>
              ...
              {activeTab === 'orders' && (
                <motion.div key="orders" className="bg-white rounded-[2.5rem] border border-outline-variant p-8 text-center py-24">
                   <Package className="w-16 h-16 text-on-surface-variant opacity-20 mx-auto mb-6" />
                   <h3 className="text-2xl font-heading font-extrabold text-primary mb-2">Order History</h3>
                   <p className="text-on-surface-variant mb-8">Detailed view of all your wellness shipments.</p>
                   <Link href="/shop" className="inline-flex py-4 px-10 bg-primary text-on-primary rounded-2xl font-bold hover:shadow-xl transition-all">
                     Continue Shopping
                   </Link>
                </motion.div>
              )}
